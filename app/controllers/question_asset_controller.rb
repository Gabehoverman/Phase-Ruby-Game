...
class QuestionAssetController < ApplicationController
	before_action :confirm_logged_in

  def index
    @photos = QuestionAsset.order("created_at desc").to_a
  ...
  def new
    @photo = QuestionAsset.new(:title => "My photo \##{1 + (QuestionAsset.maximum(:id) || 0)}")
    if !@photo
     @photo = QuestionAsset.new(question_asset_params)
    end
    if unsigned_mode?
      @unsigned = true
